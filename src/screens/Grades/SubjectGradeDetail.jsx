...
function ComponentBlock({ icon, title, weight, weighted, raw, transmuted, children }) {
  return (
    <View style={styles.compBlock}>
      <View style={styles.compBlockHeader}>
...
// settings: sheetSettings (weights/transmutation), periodSettings: per-period maxes
function PeriodSection({ component, label, settings: s, periodSettings: ps, attendance }) {
  ...
  // maxes come from grade_period_settings; fall back to grade_sheet_settings
  ...
  return (
    <View style={styles.periodSection}>
      ...
      <View style={styles.compBlocksWrap}>
        ...
        {/* 1. Attendance — shown first */}
        {s?.weight_attendance > 0 && (
          <ComponentBlock icon="calendar-outline" title="Attendance"
            weight={s.weight_attendance} weighted={component.attendance_weighted}
          >
            {attendance.length > 0 ? (
              <>
                {attendance.map((entry, i) => <AttendanceDayRow key={i} entry={entry} />)}
        ...
        {hasQuiz && (
          <>
            {s?.weight_attendance > 0 && <View style={styles.blockDivider} />}
            <ComponentBlock icon="help-circle-outline" title="Quizzes"
              weight={s?.weight_quizzes} weighted={component.quizzes_weighted}
        ...
        {hasAct && (
          <>
            {(s?.weight_attendance > 0 || hasQuiz) && <View style={styles.blockDivider} />}
            <ComponentBlock icon="create-outline" title="Activities"
              weight={s?.weight_activities} weighted={component.activities_weighted}
        ...
        {s?.weight_recitation > 0 && (
          <>
            ...
            <ComponentBlock icon="mic-outline" title="Recitation"
              weight={s.weight_recitation} weighted={component.recitation_weighted}
        ...
        {s?.weight_laboratory > 0 && (
          <>
            ...
            <ComponentBlock icon="flask-outline" title="Laboratory"
              weight={s.weight_laboratory} weighted={component.laboratory_weighted}
        ...
        {s?.weight_major_exam > 0 && (
          <>
            ...
            <ComponentBlock icon="document-text-outline" title="Major Exam"
              weight={s.weight_major_exam} weighted={component.major_exam_weighted}
        ...
        ].filter(b => b.w != null).map((b, i) => (
          <React.Fragment key={b.title}>
            {i > 0 && <View style={styles.blockDivider} />}
            <ComponentBlock icon={b.icon} title={b.title} weighted={b.w} />
          </React.Fragment>
        ))}
      </View>
    </View>
